...
/**
 * Native functions and callbacks for Salamander library.
 *
 * The functions in this class use JSON formatted strings to exchange data with the
 * native Salamander library functions.
 * 
...
public abstract class SalamanderNative { //  extends Service {  -- depends on the implementation of the real Java class 

    /**
     * Initialize the Salamander library.
     * 
     * The following native functions MUST NOT be static because their native implementation 
     * use the "this" object.
     * 
     * An application must call this functions before it can use any other Salamander library
     * functions.
     * 
     ...
     * and has no Salamander session. In this case the function interrogates the provisioning server
     * to get the necessary Salamander data of the recipient, creates a session and then sends the 
     * message.
     *
    ...
    /**
     * Request names of known trusted Salamander user identities.
     *
     * The Salamander library stores an identity (name) for each remote user.
     *
    ...
    /**
     * Request a user's Salamander device names.
     *
     * Ask the server for known Salamander devices of a user.
     *
     * @return JSON formatted information about user's Salamander devices. It returns {@code null} 
     *         if no devices known for this user.
     */
    public static native byte[] getAxoDevicesUser(byte[] userName);

    /**
     * Register Salamander device.
     *
     ...
     */
    public static native byte[] registerSalamanderDevice(int[] resultCode);

    /**
     * Remove Salamander device.
     *
     * Remove an Salamander device from a user's account.
     * 
     * @param resultCode a inte array with at least a length of one. The functions returns the
     *        request result code at index 0
     ...
     * @return a JSON string as UTF-8 encoded bytes, contains information in case of failures.
     */
    public static native byte[] removeSalamanderDevice(byte[] deviceId, int[] resultCode);
     ...
     *
     * The Salamander library uses this callback function to report message state changes to the UI.
     ...
     * 
     * The Salamander library uses this callback to perform HTTP(S) requests. The application should
     * implement this function to contact a provisioning server or other server that can return
     * required Salamander data. If the application really implements this as HTTP(S) or not is an
     * implementation detail of the application.
     * 
     * The Salamander library creates a request URI, provides request data if required, specifies the
     ...
     * Notify callback.
     *
     * The Salamander library uses this callback function to report data of a SIP NOTIFY to the app.
     *
     ...
     * -- these do not really belong to Salamander, however it's simpler
