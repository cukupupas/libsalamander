 ...
 */
int32_t AppInterfaceImpl::registerSalamanderDevice(string* result)
{
    cJSON *root;
...
int32_t AppInterfaceImpl::removeSalamanderDevice(string& devId, string* result)
{
...
// Get known Salamander device from provisioning server, check if we have a new one
// and if yes send a "ping" message to the new devices to create an Salamander conversation
// for the new devices.

// This is the ping command the code sends to new devices to create an Salamander setup
                                                       ...
                                                       const string& attachementDescriptor, const string& messageAttributes)
{
    ...
    while (!devices->empty()) {
        ...
        if (axoConv == NULL) {
            Log("++++ Salamander Conversation is NULL. Owner: %s, receipient: %s, recipientDeviceId: %s", 
