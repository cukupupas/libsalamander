...
#include "../salamander/crypto/DhPublicKey.h"
...
public:
    ...
    /**
     * @brief Register a device for use with Salamander
     * 
    ...
    /**
     * @brief Remove a Salamander device from user's account.
     * 
     * @param name the user's name
     * @param scClientDevId the unique device id of one of the user's registered Salamander devices
     * @param authorization autorization data, may be needed for some servers
     * @param result To store the result data of the server, usually in case of an error only
     */
     ...
     * 
     * @param name the user's name
     * @param longDevId the unique device id of one of the user's registered Salamander devices
     * @param authorization autorization data, may be needed for some servers
    ...
    static int32_t getPreKeyBundle(const string& name, const string& longDevId, const string& authorization, 
                                   pair< const salamander::DhPublicKey*, const salamander::DhPublicKey* >* preIdKeys );
//    static int32_t getPreKeyBundle(const std::string& name, const std::string& longDevId, const std::string& authorization);
    ...
    /**
     * @brief Get the availabe registered Salamander device of a user
     * 
     * A user may register severval devices for Salamander usage. A sender (Alice) should send messages to
     ...
     *
     * @param store The persitent Salamander store to store and retrieve state information.
     * @param longDevId the unique device id of one of the user's registered Salamander devices
