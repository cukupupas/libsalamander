...
#include "../salamander/Constants.h"
...
// /v1/me/device/<device_id>/salamander/keys/?api_key=<API_key>
// Method: PUT

static const char* registerRequest = "/v1/me/device/%s/salamander/keys/?api_key=%s";
...
// Implementation of the Provisioning API: remove an Salamander Device 
// /v1/me/device/<device_id>/salamander/keys/?api_key=<API_key>
// Method: DELETE
...
int32_t Provisioning::getPreKeyBundle(const string& name, const string& longDevId, const string& authorization, pair<const DhPublicKey*, const DhPublicKey*>* preIdKeys)
{
    ...
    cJSON* cjKey = cJSON_GetObjectItem(root, "salamander");
    if (cjKey == NULL) {
        return 0;
    }
...
int32_t Provisioning::getNumPreKeys(const string& longDevId,  const string& authorization)
{
    ...
    cJSON* salamander = cJSON_GetObjectItem(root, "salamander");
    if (salamander == NULL) {
        cJSON_Delete(root);
        return -1;
    }

    cJSON* keyIds = cJSON_GetObjectItem(salamander, "prekeys");
...
// Implementation of the Provisioning API: Get Available Salamander registered devices of a user
// Request URL: /v1/user/wernerd/devices/?filter=salamander&api_key=<apikey>
// Method: GET
/*
 {
    "version" :        <int32_t>,        # Version of JSON new pre-keys, 1 for the first implementation
    {"devices": [{"version": 1, "id": <string>, "device_name": <string>}]}  # array of known Salamander ScClientDevIds for this user/account
 }
 */
static const char* getUserDevicesRequest = "/v1/user/%s/device/?filter=salamander&api_key=%s";
